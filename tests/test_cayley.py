...
from bella.matrix import matmul
import mpmath as mp
...
def test_generators_from_circle_inversions():
    ...
    assert matrix_almosteq_up_to_sign(matmul(orthogonal_gens[0], orthogonal_gens[0]), mp.eye(2))
    ...
    assert len(gens1)==1
    assert len(gens2)==1
    assert matrix_almosteq_up_to_sign(matmul(gens1[0], gens2[0]), mp.eye(2))
    ...
    projective_point = matmul(gens2[0], mp.matrix([[4+3j],[1]]))
    assert mp.fabs(projective_point[1]/projective_point[0] - circle_1[0]) < circle_1[1]
    projective_point = matmul(gens2[0], mp.matrix([[0],[1]]))
    assert mp.fabs(projective_point[1]/projective_point[0] - circle_1[0]) < circle_1[1]
    projective_point = matmul(gens2[0], mp.matrix([[1j],[1]]))
    assert mp.fabs(projective_point[1]/projective_point[0] - circle_1[0]) < circle_1[1]
