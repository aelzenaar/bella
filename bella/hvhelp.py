...
default_opts = {"fill_alpha":0, "line_width":1}

def makeCircles(*args,**kwargs):
    return Circles(*args,**kwargs).opts(**default_opts)
...
def pairsToCircles(pairs):
    ...
    return makeCircles(df, kdims = ['x'], vdims = ['y','radius']).opts(radius='radius')
